...
std::array<int, 2> AIPlayer::moveAI() {
	...
	if (game.getBoard().getRemainPlaces() == BOARD_SZ*BOARD_SZ)
		...
		std::tuple<Node*, int> move = getStats<std::tuple<Node*, int>, Game &, Node &, int, int, bool, int, int>("miniMax", miniMax, game, node, depth, THR_DEPTH, true, std::numeric_limits<int>::min(), std::numeric_limits<int>::max());
